...
import { Loader2, Search, Plus, Youtube, FileText, AlertCircle, Check, Trash2, ExternalLink, Video } from 'lucide-react';
...
import OcuMonitor from '@/pages/OcuMonitor';

// Tipos de pestañas
type TabType = 'youtube' | 'ocu';
...
const ControlHaters = () => {
  ...
  // ID de la empresa que tiene acceso a OCU Monitor
  const OCU_MONITOR_COMPANY_ID = '5';
  ...
  useEffect(() => {
    if (companyId && activeTab === 'youtube') {
  ...
  // Filtrar pestañas según empresa
  const availableTabs = tabs.filter(tab => {
    if (tab.id === 'ocu') {
      return companyId === OCU_MONITOR_COMPANY_ID;
    }
    return true;
  });
    ...
    <div className="min-h-screen max-h-screen overflow-hidden flex">
      ...
      <div className="flex-1 overflow-y-auto relative z-10 scrollbar-thin scrollbar-thumb-slate-700/50 scrollbar-track-slate-900/50 hover:scrollbar-thumb-slate-600/70">
        <div className="p-3 sm:p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-4 sm:space-y-6 lg:space-y-8">
          ...
          {/* Sistema de Pestañas - SOLO SI HAY MÁS DE UNA */}
          {availableTabs.length > 1 && (
            <div className="relative">
              ...
              <div className="relative p-2 glass-effect rounded-xl sm:rounded-2xl">
                <div className="flex gap-2 overflow-x-auto scrollbar-thin scrollbar-thumb-slate-700/50 scrollbar-track-transparent">
                  {availableTabs.map((tab) => (
                    <button
                      key={tab.id}
                      onClick={() => setActiveTab(tab.id)}
                      className={`
                        relative flex items-center gap-2 px-4 sm:px-6 py-3 rounded-lg
                        transition-all duration-300 whitespace-nowrap
                        ${activeTab === tab.id 
                          ? `bg-gradient-to-r ${tab.color} text-white shadow-lg` 
                          : 'glass-effect text-white/70 hover:text-white hover:bg-white/5'
                        }
                      `}
                    >
                      {tab.icon}
          ...
          ) : activeTab === 'ocu' && companyId === OCU_MONITOR_COMPANY_ID ? (
            <OcuMonitor />
          ) : null}
        </div>
      </div>
